#!/usr/bin/env python3
"""
Download USFS Conditional Flame Length data from ImageServer and tile at 0.1 degrees.

Source: USFS Wildfire Risk to Communities (WRC) Conditional Flame Length 2024
  https://imagery.geoplatform.gov/iipp/rest/services/Fire_Aviation/USFS_EDW_RMRS_WRC_ConditionalFlameLength/ImageServer

The raster values are uint16 representing conditional flame length in tenths of feet
(range 0–4750, i.e., 0–475 feet). CFL is the expected flame length given that a fire
occurs; it is NOT the probability of fire. Values will be 0 at developed/non-burnable
locations — that is expected.

We store the raw uint16 values. The Worker divides by 10 at query time to get feet.
"""
import argparse
import json
import math
...
BASE_URL = "https://imagery.geoplatform.gov/iipp/rest/services/Fire_Aviation/USFS_EDW_RMRS_WRC_ConditionalFlameLength/ImageServer/exportImage"
...
def write_tile(data, path, lat_south, lon_west, rows, cols):
    """Write uint16 tile with JSON sidecar (same format as burn_probability)."""
    path = Path(path)
    ...
    sidecar = {
        ...
        "units": "conditional_flame_length_ft",
        "dtype": "uint16",
    }
...
def main():
    parser = argparse.ArgumentParser(description="Download conditional flame length tiles")
    parser.add_argument("--output-dir", default="data/tiles/cfl")
